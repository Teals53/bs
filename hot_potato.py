...
class HotPotato(ba.TeamGameActivity[Player, ba.Team]):
    ...
    # Update the gamemode's name to differentiate between normal or Impact bomb variants.
    @override
    @classmethod
    def get_display_string(cls, settings: dict | None = None) -> babase.Lstr:
        name = ba.Lstr(translate=('gameNames', cls.getname()))
        if settings is not None:
            if 'Marked Players use Impact Bombs' in settings and settings['Marked Players use Impact Bombs']:
